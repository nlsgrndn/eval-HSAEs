...
class SAELoss(torch.nn.Module):
    ...
    def __init__(
        ...
        ewg_sae_groups: Optional[list[int]] = None,
    ):
        """
        ...
        self.ewg_sae_groups = ewg_sae_groups
    ...
    def forward(
        ...
        if self.ewg_sae_groups is not None and len(self.ewg_sae_groups) > 0:
            # If using EWGSAE, compute sparsity loss separately for each group and average
            group_losses = []
            start_idx = 0
            exponential_weights = [0.5 ** i for i in range(len(self.ewg_sae_groups))]
            exponential_weights = exponential_weights[::-1]  # Reverse to lower smaller, earlier groups
            for end_idx in self.ewg_sae_groups:
                group_activations = latent_activations[:, start_idx:end_idx]
                group_loss = self.sparse_loss_fn(group_activations, original_input)
                group_losses.append(group_loss)
                start_idx = end_idx
            group_losses = torch.stack(group_losses)
            weights_tensor = torch.tensor(exponential_weights, device=group_losses.device)
            sparse_loss = (group_losses * weights_tensor).sum() / weights_tensor.sum()
        else:
