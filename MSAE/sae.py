...
class Autoencoder(nn.Module):
    ...
    def __init__(
        ...
        *args, **kwargs
        ...
        if isinstance(activation, str):
            ...
            # Handle legacy case or nn.Module instances
            activation_str = type(activation).__name__ if hasattr(activation, '__name__') else "ReLU"
        
        # Store initialization config (all args needed to reconstruct)
        self._init_config = {
            'n_latents': n_latents,
            'n_inputs': n_inputs,
            ...
            'tied': tied,
            'normalize': normalize,
            'bias_init': 0.0 if isinstance(bias_init, torch.Tensor) else bias_init,
        ...
        #self.dead_activations = activation # was in original code but seems unused
    ...
    def scale_to_unit_norm(self) -> None:
        ...
        if self.tied:
            ...
            normalized = self.encoder.data.T / norm
            self.encoder.data = normalized.T
        else:
    ...
    def get_primary_outputs(self, forward_output):
        """Extract (recons, latents, all_recons, all_latents) as tensors."""
        return forward_output  # Already in right format


class EWGSAE(Autoencoder):
    """
    Exponentially Weighted Group Sparse Autoencoder.
    """
    def __init__(
        self, n_latents: int, n_inputs: int, activation: str = "ReLU", tied: bool = False, normalize: bool = False,
        bias_init: torch.Tensor | float = 0.0, init_method: str = "kaiming", latent_soft_cap: float = 30.0,
        ewg_sae_groups: list[int] = [], *args, **kwargs
    ) -> None:
        """
        ...
        Args:
            ...
            init_method (str, optional): Weight initialization method. Defaults to "kaiming".
            latent_soft_cap (float, optional): Soft cap value for latent activations. Defaults to 30.0.
            ewg_sae_groups (list[int], optional): List of group sizes for EWGSAE. Defaults to [].
        """

        super().__init__(n_latents, n_inputs, activation, tied, normalize, bias_init, init_method, latent_soft_cap)

        # Ensure activation is TopK-based
        if "TopK"  in activation:
            raise ValueError(f"EWGSAE requires a non-TopK activation function, but got {activation}. Please choose a different activation (e.g., ReLU).")

        self.ewg_sae_groups = ewg_sae_groups  # cumulative end indices, set by train.py before model creation

        # Store complete config (including parent params + MultiSAE-specific params)
        self._init_config = {
            'n_latents': n_latents,
            'n_inputs': n_inputs,
            ...
            'init_method': init_method,
            'latent_soft_cap': latent_soft_cap,
            'ewg_sae_groups': ewg_sae_groups,
        }
        
        
    # @torch.no_grad()
    # def project_grads_decode(self):
    #     # ── First attempt (full cross-feature projection — wrong for over-complete dicts) ──
    #     # weights = self.decoder.data   # (n_latents, n_inputs)
    #     # grad = self.decoder.grad      # (n_latents, n_inputs)
    #     # self.decoder.grad -= (grad @ weights.T) @ weights
    #     # ────────────────────────────────────────────────────────────────────────────────────
    #
    #     # Cross-group asymmetric gradient projection (EWG-SAE paper §3, eq. 3).
    #     warmup_factor = getattr(self, '_grad_proj_warmup_factor', 1.0)
    #     weights = self.decoder.data          # (n_latents, n_inputs), unit-norm rows
    #     grad = self.decoder.grad             # (n_latents, n_inputs)
    #     groups = self.ewg_sae_groups         # cumulative end indices [g0, g1, ..., gK]
    #     boundaries = [0] + list(groups)
    #     K = len(groups)
    #     for k in range(1, K):
    #         start_k = boundaries[k]
    #         end_k = boundaries[k + 1]
    #         n_broader = boundaries[k]
    #         n_inputs = weights.shape[1]
    #         if n_broader == 0:
    #             continue
    #         if n_broader >= n_inputs:
    #             continue
    #         B_k = weights[:n_broader, :].detach()
    #         Q_k, _ = torch.linalg.qr(B_k.T, mode='reduced')
    #         G_k = grad[start_k:end_k, :]
    #         proj = (G_k @ Q_k) @ Q_k.T
    #         grad[start_k:end_k, :] -= warmup_factor * proj
    #     super().project_grads_decode()
    ...
    def encode(self, x: torch.Tensor, topk_number: int | None = None) -> tuple[torch.Tensor, torch.Tensor, dict[str, Any]]:
        ...
        Returns:
            tuple: (encoded, full_encoded, info)
                ...
                - info: Normalization information dictionary
        """
        return super().encode(x, topk_number)
    ...
    def decode(self, latents: torch.Tensor, info: dict[str, Any] | None = None) -> torch.Tensor:
        ...
        Returns:
            torch.Tensor: Reconstructed input data [batch, n_inputs]
        """
        return super().decode(latents, info)
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        Returns:
            tuple: (recons, latents, all_recons, all_latents)
                - recons: List of reconstructed data at different sparsity levels
                - latents: List of latent activations at different sparsity levels
                - all_recons: List of reconstructions without sparsity constraints (for analysis)
                - all_latents: List of latent activations without sparsity constraints (for analysis)

        """
        return super().forward(x)

    @torch.no_grad()
    def forward_topk_eval(self, x: torch.Tensor, k: int = 64) -> tuple[torch.Tensor, torch.Tensor]:
        """
        Eval-only forward pass that keeps only the top-k activations per sample.

        Mirrors forward() but inserts a hard top-k filter on the post-activation
        latents before decoding. Not used for training; does not update latent
        statistics.

        Args:
            x: Input data [batch, n_inputs]
            k: Number of activations to keep per sample (default 64)

        Returns:
            tuple: (recons_topk, latents_topk)
        """
        x_processed, info = self.preprocess(x)
        latents_pre_act = self.encode_pre_act(x_processed)
        latents = self.activation(latents_pre_act)

        _, indices = torch.topk(latents, k=k, dim=-1)
        values = torch.gather(latents, -1, indices)
        latents_topk = torch.zeros_like(latents)
        latents_topk.scatter_(-1, indices, values)

        latents_topk = self.latent_soft_cap(latents_topk)
        recons_topk = self.decode(latents_topk, info)
        return recons_topk, latents_topk

    def get_primary_outputs(self, forward_output):
        """Extract (recons, latents, all_recons, all_latents) as tensors."""
        return super().get_primary_outputs(forward_output)

class MatchingPursuitSAE(Autoencoder):
    """
    Matching Pursuit SAE from https://arxiv.org/abs/2506.03093 (repo: https://github.com/mpsae/MP-SAE)
    """
    def __init__(
        self, n_latents: int, n_inputs: int, activation: str = "TopKReLU", tied: bool = True, normalize: bool = False,
        bias_init: torch.Tensor | float = 0.0, init_method: str = "kaiming", latent_soft_cap: float = 30.0, mpsae_mode: str = "TOPK", mpsae_mode_hyperparam: float = 64, *args, **kwargs
    ) -> None:
        """
        Initialize the Matching Pursuit Sparse Autoencoder.
        ...
        assert tied == True, "Matching Pursuit SAE requires tied weights"
        assert latent_soft_cap == 0.0, "Matching Pursuit SAE requires latent_soft_cap = 0.0"
        # Initialize with activation
        assert mpsae_mode in ["TOPK", "THRESHOLD"], f"Invalid mode {mpsae_mode} for MatchingPursuitSAE"
        self.mode = mpsae_mode
        self.mode_hyperparam = mpsae_mode_hyperparam if mpsae_mode == "THRESHOLD" else int(mpsae_mode_hyperparam)

        super().__init__(n_latents, n_inputs, activation, tied, normalize, bias_init, init_method, latent_soft_cap)
        
        # Store complete config (including parent params + MPSAE-specific params)
        self._init_config = {
            'n_latents': n_latents,
            'n_inputs': n_inputs,
            'activation': activation,
            ...
            'init_method': init_method,
            'latent_soft_cap': latent_soft_cap,
            'mpsae_mode': mpsae_mode,
            'mpsae_mode_hyperparam': mpsae_mode_hyperparam,
        }

    def encode(self, x: torch.Tensor, topk_number: int | None = None) -> tuple[list[torch.Tensor], torch.Tensor, dict[str, Any]]:
        raise NotImplementedError("MatchingPursuitSAE encode method not implemented yet.")

    def decode(self, latents: torch.Tensor, reconstruct_size: tuple, info: dict[str, Any] | None = None) -> list[torch.Tensor]:
        raise NotImplementedError("MatchingPursuitSAE decode method not implemented yet.")

    def _forward_threshold(self, x: torch.Tensor) -> torch.Tensor:
        THRESHOLD = self.mode_hyperparam
        residual = x.clone()
        batch_size = x.shape[0]

        device = x.device

        z = torch.zeros(batch_size, self.n_latents, device=device)
        prev_support = torch.zeros_like(z).bool()
        done = torch.zeros(batch_size, dtype=torch.bool, device=device)

        while not done.all():
            WTr = torch.relu(residual @ self.encoder)
            ...
            update = torch.matmul(z_, self.encoder.T)
    ...
    def forward(self, x: torch.Tensor) -> tuple[list[torch.Tensor], list[torch.Tensor], torch.Tensor, torch.Tensor]:
        ...
        Returns:
            tuple: (recons, latents, all_recons, all_latents)
                - recons: Reconstructed data [batch, n_inputs] without sparsity constraints as it inherently has no sparsity constraints
                - latents: Latent activations [batch, n_latents] without sparsity constraints as it inherently has no sparsity constraints
                - all_recons: Reconstructed data [batch, n_inputs] without sparsity constraints
                - all_latents: Latent activations [batch, n_latents] without sparsity constraints
        """
        x, info = self.preprocess(x)
        x = x - self.pre_bias
        # latents_pre_act_full = x @ self.encoder + self.latent_bias
        if self.mode == "TOPK":
            z = self._forward_topk(x)
        elif self.mode == "THRESHOLD":
            z = self._forward_threshold(x)
        else:
            raise ValueError(f"Invalid mode {self.mode} for MatchingPursuitSAE")
        
        x_hat = torch.matmul(z, self.encoder.T)
        x_hat= x_hat + self.pre_bias

        encoded = z
        caped_encoded = encoded
        recons = x_hat

        # Update activation statistics
        self.update_latent_statistics(caped_encoded)

        # Return reconstructions and latents
        return recons, caped_encoded, recons, caped_encoded
    
    def get_primary_outputs(self, forward_output):
        """Extract (recons, latents, all_recons, all_latents) as tensors."""
        return forward_output  # Already in right format; BUT NOTE: all is the same as sparse version

class MultiSAE(Autoencoder):
    """
    Multi Sparse Autoencoder.

    This extends the base Autoencoder with independent SAEs.
    
    """
    def __init__(
        self, n_latents: int, n_inputs: int, activation: str = "TopKReLU", tied: bool = False, normalize: bool = False,
        bias_init: torch.Tensor | float = 0.0, init_method: str = "kaiming", latent_soft_cap: float = 30.0,
        group_fractions: list[float] = [1/16, 1/8, 1/4, 1/2 + 1/16], relative_importance: list[float] | None = None, nesting_list: list[int] = [16, 32, 64, 128], *args, **kwargs
    ) -> None:
        """
        Initialize the Matryoshka Sparse Autoencoder.
        ...
        Args:
            ...
            init_method (str, optional): Weight initialization method. Defaults to "kaiming".
            latent_soft_cap (float, optional): Soft cap value for latent activations. Defaults to 30.0.
            group_fractions (list[float], optional): Fractions of latent space for each group. Defaults to [1/16, 1/8, 1/4, 1/2 + 1/16].
            relative_importance (list[float] | None, optional): Importance weights for each nesting level.
                                                              Defaults to equal weights.
            nesting_list (list[int], optional): List of nesting levels (k values) for TopK activations.
        """
        # Initialize hierarchy
        self.num_groups = len(group_fractions)
        self.group_fractions = group_fractions
        self.group_sizes = [int(f * n_latents) for f in group_fractions]
        self.relative_importance = relative_importance if relative_importance is not None else [1.0] * self.num_groups
        self.group_starts = [0] + list(np.cumsum(self.group_sizes)[:-1])
        self.nesting_list = sorted(nesting_list)
        assert len(self.relative_importance) == self.num_groups
        assert len(self.nesting_list) == self.num_groups

        # Ensure activation is TopK-based
        if "TopK" not in activation:
            warnings.warn(f"MultiSAE: activation {activation} is not a TopK activation. We are changing it to TopKReLU")
            activation = "TopKReLU"
    ...
    def encode(self, x: torch.Tensor, topk_number: int | None = None) -> tuple[list[torch.Tensor], torch.Tensor, dict[str, Any]]:
        ...
        encoded = [activation(pre_encoded[:, group_start_idx:group_start_idx + group_size]) for activation, group_start_idx, group_size in zip(
            self.activation,
            self.group_starts,
            self.group_sizes
        )]
        ...
        if topk_number is not None:
            raise NotImplementedError("MultiSAE encode method does not support topk_number yet.")
        last_encoded = caped_encoded[-1] 
        
        return caped_encoded, last_encoded, info

    def decode(self, latents: list[torch.Tensor], reconstruct_size: tuple, info: dict[str, Any] | None = None) -> list[torch.Tensor]:
        """
        ...
        Args:
            ...
            reconstruct_size (tuple): Size of the reconstructed outputs (should be [batch_size, embedding_size])
            info (dict[str, Any] | None, optional): Normalization information. Defaults to None.
        ...
        W_dec_chunks = torch.split(self.decoder, self.group_sizes, dim=0)

        # shape of x_reconstructs is self.num_groups x reconstruct_size
        x_reconstructs = torch.zeros((self.num_groups,) + reconstruct_size, device=latents[0].device, dtype=latents[0].dtype)
        for i, latent_subset in enumerate(latents):
            W_dec_slice = W_dec_chunks[i]
            x_reconstructs[i] = latent_subset @ W_dec_slice

        # add the bias
        x_reconstructs = x_reconstructs + self.pre_bias

        return x_reconstructs # shape: self.num_groups x reconstruct_size
    ...
    def forward(self, x: torch.Tensor) -> tuple[list[torch.Tensor], list[torch.Tensor], list[torch.Tensor], list[torch.Tensor]]:
        ...
        Returns:
            tuple: (recons, latents, recons_all, latents_all)
                - recons: reconstructions for each sae (list of tensors with shape [batch, n_inputs])
                - latents: latent activations for each sae (list of tensors with shape [batch, n_latents] where n_latents differs per sae)
                - recons_all: Reconstruction from each sae without topk applied (list of tensors with shape [batch, n_inputs])
                - latents_all: Latent activations for each sae without topk applied (list of tensors with shape [batch, n_latents] where n_latents differs per sae)
        """
        # # Preprocess data
        # caped_encoded, _, info = self.encode(x)

        # Manually implement encode to get all intermediate reconstructions
        x, info = self.preprocess(x)
        encoded_pre_act = self.encode_pre_act(x)
        
        encoded = [activation(encoded_pre_act[:, group_start_idx:group_start_idx + group_size]) for activation, group_start_idx, group_size in zip(
            self.activation,
            self.group_starts,
            self.group_sizes
        )]
        ...
        reconstruct_size = x.shape
        # Decode each level
        recons = self.decode(caped_encoded, reconstruct_size, info)
        
        # Update activation statistics
        concatenated_latents = torch.cat(caped_encoded, dim=-1)
        self.update_latent_statistics(concatenated_latents)

        # Latents and recons without applying topk
        encoded_no_topk = [activation.forward_eval(encoded_pre_act[:, group_start_idx:group_start_idx + group_size]) for activation, group_start_idx, group_size in zip(
            self.activation,
            self.group_starts,
            self.group_sizes
        )]
        caped_encoded_no_topk = [self.latent_soft_cap(enc) for enc in encoded_no_topk]
        all_recons = self.decode(caped_encoded_no_topk, reconstruct_size, info)
        
        # Return all reconstructions and the final ones
        return recons, caped_encoded, all_recons, caped_encoded_no_topk
    
    def get_primary_outputs(self, forward_output, sae_index: int = -1):
        """Extract (recons, latents, all_recons, all_latents) as tensors."""
        recons, latents, all_recons, all_latents = forward_output
        # return recons[sae_index], latents[sae_index], all_recons[sae_index], all_latents[sae_index]
        # concatenate the latents into a single tensor; # use the sae_index to select the reconstructions
        latents_concat = torch.cat(latents, dim=-1)
        all_latents_concat = torch.cat(all_latents, dim=-1)
        return recons[sae_index], latents_concat, all_recons[sae_index], all_latents_concat


class ArchitecturalMatryoshkaSAE(Autoencoder):
    """
    Architectural Matryoshka Sparse Autoencoder.

    This extends the base Autoencoder with a architecturally nested structure of latent representations,
    ...
    def __init__(
        self, n_latents: int, n_inputs: int, activation: str = "TopKReLU", tied: bool = False, normalize: bool = False,
        bias_init: torch.Tensor | float = 0.0, init_method: str = "kaiming", latent_soft_cap: float = 30.0,
        group_fractions: list[float] = [1/16, 1/8, 1/4, 1/2 + 1/16], relative_importance: list[float] | None = None, *args, **kwargs
    ) -> None:
        """
        Initialize the Matryoshka Sparse Autoencoder.
        ...
        # Store complete config (including parent params + ArchMatryoshka-specific params)
        self._init_config = {
            'n_latents': n_latents,
            'n_inputs': n_inputs,
            'activation': activation,
    ...
    def encode(self, x: torch.Tensor, topk_number: int | None = None) -> tuple[torch.Tensor, dict[str, Any]]:
        """
        ...
        Returns:
            tuple: (caped_encoded,info)
                - caped_encoded: latent activations 
                - info: Normalization information dictionary
        """
        x, info = self.preprocess(x)
        pre_encoded = self.encode_pre_act(x)
        ...
        if topk_number is not None:
            _, indices = torch.topk(caped_encoded, k=topk_number, dim=-1)
            values = torch.gather(caped_encoded, -1, indices)
            caped_encoded = torch.zeros_like(caped_encoded)
            caped_encoded.scatter_(-1, indices, values)

        return caped_encoded, info
    ...
    def decode(self, latents: torch.Tensor, reconstruct_size: tuple, info: dict[str, Any] | None = None) -> list[torch.Tensor]:
        ...
        Args:
            latents (torch.Tensor): latent activations 
            reconstruct_size (tuple): Size of the reconstructed outputs (should be [batch_size, embedding_size])
            info (dict[str, Any] | None, optional): Normalization information. Defaults to None.
        
        Returns:
            torch.Tensor: reconstructed inputs at different latent prefixes with additional dimension for num_groups
        """

        # We could potentially refactor the ae class to use W_dec_chunks instead of W_dec, may be more efficient
        W_dec_chunks = torch.split(self.decoder, self.group_sizes, dim=0)
        f_chunks = torch.split(latents, self.group_sizes, dim=1)

        # shape of x_reconstructs is self.num_groups x reconstruct_size
        x_reconstructs = torch.zeros((self.num_groups,) + reconstruct_size, device=latents.device, dtype=latents.dtype)
        for i in range(self.num_groups):
            W_dec_slice = W_dec_chunks[i]
            acts_slice = f_chunks[i]
            x_reconstructs[i] = acts_slice @ W_dec_slice
        # cumulate along num_groups axis
        x_reconstructs = x_reconstructs.cumsum(dim=0)
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        Returns:
            tuple: (recons_per_prefix, latents, all_recons, all_latents)
                - recons_per_prefix: reconstructions at different prefixes of latent activations [num_groups, batch, n_inputs]
                - latents: latent activations with topk applied [batch, n_latents]
                - all_recons: Reconstruction without sparsity constraints (for analysis) [batch, n_inputs]
                - all_latents: Latent activations without sparsity constraints (for analysis) [batch, n_latents]
        """
        # # Preprocess data
        # caped_encoded, info = self.encode(x)
        ...
        # Apply activation function
        encoded = self.activation(encoded_pre_act)
        caped_encoded = self.latent_soft_cap(encoded)
        ...
        # Latents and recons without applying topk
        all_latents = self.activation.forward_eval(encoded_pre_act)
        all_latents_caped = self.latent_soft_cap(all_latents)
        all_recons = self.decode(all_latents_caped, reconstruct_size, info)[-1]
        
        # Return all reconstructions and the final ones
        return recons, caped_encoded, all_recons, all_latents_caped
    
    def get_primary_outputs(self, forward_output):
        """Extract (recons, latents, all_recons, all_latents) as tensors."""
        recons, latents, all_recons, all_latents = forward_output
        return recons[-1], latents, all_recons, all_latents
...
class MatryoshkaAutoencoder(Autoencoder):
    ...
    def __init__(
        ...
        # Store complete config (including parent params + Matryoshka-specific params)
        self._init_config = {
            'n_latents': n_latents,
            'n_inputs': n_inputs,
            'activation': activation,
    ...
    def forward(self, x: torch.Tensor) -> tuple[list[torch.Tensor], list[torch.Tensor], torch.Tensor, torch.Tensor]:
        ...
        Returns:
            tuple: (recons_list, latents_list, final_recon, final_latent)
                - recons_list: List of reconstructions at different sparsity levels (list of tensors with shape [batch, n_inputs])
                - latents_list: List of latent activations at different sparsity levels (list of tensors with shape [batch, n_latents])
                - final_recon: Reconstruction from the largest k value [batch, n_inputs]
                - final_latent: Latent activations from the largest k value [batch, n_latents]
        """
        # Preprocess data
        x_processed, info = self.preprocess(x)
        latents_pre_act = self.encode_pre_act(x_processed)
    ...
    def get_primary_outputs(self, forward_output):
        """Extract (recons, latents, all_recons, all_latents) as tensors."""
        recons_list, latents_list, all_recons, all_latents = forward_output
        return recons_list[0], latents_list[0], all_recons, all_latents



# Legacy comment kept for reference:
# Removed SAE class as it only serves as a wrapper around Autoencoder and is redundant with the potential for mistakes
# class SAE(nn.Module):
