...
class TrainConfig:
    ...
    Attributes:
        ...
        ewg_warmup_steps (int): Number of steps to warm up sparse_weight from 0 to its final value (EWG-SAE only).
    """
    lr: float = 0.001
    seed: int = 42
...
class ModelConfig:
    ...
    Attributes:
        use_arch_matryoshka (bool): Whether to use architectural matryoshka (nested) architecture.
        group_fractions (list[float]): Fractions of latent space for each group.
    ...
    """
    use_ewgsae: bool = False
    use_multisae: bool = False
    use_mpsae: bool = False
    use_arch_matryoshka: bool = False
    ewg_sae_groups: Union[List[int], int] = 64
    group_fractions: Optional[list[float]] = None
    use_matryoshka: bool = False
    n_inputs: int = 768
    n_latents: int = -1 # to be set based on dataset
    activation: str = "ReLU"
    tied: bool = False
    ...
    max_nesting: int = 6144
    mpsae_mode: str = "TOPK"
    mpsae_mode_hyperparam: float = 64
...
def default_arch_msae_uw_training() -> TrainConfig:
    return TrainConfig(lr=1e-4)

def default_arch_msae_uw_loss() -> LossConfig:
    return LossConfig(sparse_weight=0.0)

def default_arch_msae_uw_model() -> ModelConfig:
    return ModelConfig(
        use_arch_matryoshka=True,
        group_fractions=[1/64, 1/32, 1/16, 1/8, 1/4, 1/2 + 1/64],
        activation="TopKReLU",
        relative_importance="UW",
    )

@dataclasses.dataclass
class ArchMSAE_UWConfig:
    """
    Configuration for Architected Matryoshka Sparse Autoencoder with Uniform Weighting.
    
    This configuration builds upon MSAE_UW by introducing architectural enhancements
    for improved feature learning and representation.
    """
    training: TrainConfig = field(default_factory=default_arch_msae_uw_training)
    loss: LossConfig = field(default_factory=default_arch_msae_uw_loss)
    model: ModelConfig = field(default_factory=default_arch_msae_uw_model)


def default_arch_msae_fw_training() -> TrainConfig:
    return TrainConfig(lr=1e-4)

def default_arch_msae_fw_loss() -> LossConfig:
    return LossConfig(sparse_weight=0.0)

def default_arch_msae_fw_model() -> ModelConfig:
    return ModelConfig(
        use_arch_matryoshka=True,
        group_fractions=[1/64, 1/32, 1/16, 1/8, 1/4, 1/2 + 1/64],
        activation="TopKReLU",
        relative_importance="FW",
    )

@dataclasses.dataclass
class ArchMSAE_FWConfig:
    """
    Configuration for Architected Matryoshka Sparse Autoencoder with Forward Weighting.
    
    This configuration builds upon MSAE_FW by introducing architectural enhancements
    for improved feature learning and representation.
    """
    training: TrainConfig = field(default_factory=default_arch_msae_fw_training)
    loss: LossConfig = field(default_factory=default_arch_msae_fw_loss)
    model: ModelConfig = field(default_factory=default_arch_msae_fw_model)


def default_mpsae_training() -> TrainConfig:
    return TrainConfig(lr=1e-4)

def default_mpsae_loss() -> LossConfig:
    return LossConfig(sparse_weight=0.0)

def default_mpsae_model() -> ModelConfig:
    return ModelConfig(
        use_mpsae=True,
        tied=True,
        mpsae_mode="TOPK",
        mpsae_mode_hyperparam=64,
    )
...
class MPSAEConfig:
    ...
    for improved feature learning and representation.
    """
    training: TrainConfig = field(default_factory=default_mpsae_training)
    loss: LossConfig = field(default_factory=default_mpsae_loss)
    model: ModelConfig = field(default_factory=default_mpsae_model)

def default_multisae_training() -> TrainConfig:
    return TrainConfig(lr=1e-4)

def default_multisae_loss() -> LossConfig:
    return LossConfig(sparse_weight=0.0)

def default_multisae_model() -> ModelConfig:
    return ModelConfig(
        use_multisae=True,
        nesting_list=[16, 32, 64, 128],
        group_fractions=[1/16, 1/8, 1/4, 1/2 + 1/16],
        activation="TopKReLU",
        relative_importance="UW",
    )

@dataclasses.dataclass
class MultiSAEConfig:
    """
    Configuration for Multi Sparse Autoencoder.
    """
    training: TrainConfig = field(default_factory=default_multisae_training)
    loss: LossConfig = field(default_factory=default_multisae_loss)
    model: ModelConfig = field(default_factory=default_multisae_model)


def default_ewgsae_training() -> TrainConfig:
    return TrainConfig(lr=1e-4, bias_init_median=True, ewg_warmup_steps=1000)

def default_ewgsae_loss() -> LossConfig:
    return LossConfig(sparse_weight=1.0)

def default_ewgsae_model() -> ModelConfig:
    return ModelConfig(
        use_ewgsae=True,
        ewg_sae_groups=64,
    )
...
class EWGSAEConfig:
    """
    Configuration for Exponentially Weighted Group Sparse Autoencoder.
    """
    training: TrainConfig = field(default_factory=default_ewgsae_training)
    loss: LossConfig = field(default_factory=default_ewgsae_loss)
    model: ModelConfig = field(default_factory=default_ewgsae_model)
...
def get_config(model_name: str):
    ...
    elif model_name == "ArchMSAE_UW":
        return ArchMSAE_UWConfig()
    elif model_name == "ArchMSAE_FW":
        return ArchMSAE_FWConfig()
    elif model_name == "MPSAE":
        return MPSAEConfig()
    elif model_name == "MultiSAE":
        return MultiSAEConfig()
    elif model_name == "EWGSAE":
        return EWGSAEConfig()
    else:
